...
awesome.quit = function()
    ...
    if session == "awesome-gnome" then
        os.execute("/usr/bin/gnome-session-quit")
    elseif session == "xfce" then
        os.execute("/usr/bin/xfce4-session-logout")
    else
        _awesome_quit()
    end
end
...
if awesome.startup_errors then
    os.execute('notify-send -u critical "Oops, there were errors during startup!" "' .. awesome.startup_errors .. '"')
...
do
    ...
    awesome.connect_signal("debug::error", function (err)
        ...
        naughty.notify({ preset = naughty.config.presets.critical,
                         ...
                         text = err })
        os.execute('notify-send -u critical "Oops, an error happened!" "' .. err .. '"')
        in_error = false
    end)
end
-- }}}
...
--    tags[s] = awful.tag({ "web", 2, 3, 4, 5, 6, 7, 8, 9 }, s, layouts[1])
--end
-- }} DEFAULT

tags = {}
if screen.count() == 1 then
    tags[1] = awful.tag({"Web", "IM", "IDE","Term", 5, "Email"}, 1, layouts[2])
else
    tags[1] = awful.tag({"Web","IM", "IDE","Term", 5, 6}, 1, layouts[2])
    tags[2] = awful.tag({"Email", 2, 3, 4}, 2, layouts[2])
...
-- {{{ Key bindings
function xfce_switch_action(action)
    f = function(s)
        action(s)
        c = client.focus

        if c and c.name == 'xfce4-panel' then
...
globalkeys = awful.util.table.join(
    -- CUSTOM KEYBINDINGS
    awful.key({ modkey,           }, "F12", function() awful.util.spawn("gnome-screensaver-command -l ; light-locker-command -l") end),
        ...
        awful.key({ modkey }, "#" .. i + 9,
                  function ()
                        ...
                        if tag then
                           ...
                           xfce_switch_action(function(s) end)(nil)
                        end
                  end),
        -- Toggle tag.
...
email_rule = { }
if screen.count() == 1 then
    email_rule = { rule = { instance = "outlook.office365.com__owa" },
